  ...
  <footer class="bg-secondary">
    ...
    <div class="mx-auto max-w-7xl overflow-hidden py-12 px-4 sm:px-6 lg:px-8">
      <nav
        ...
        aria-label="Footer"
      >
        <div v-for="nav in navigation" :key="nav.name" class="px-5 py-2">
          <a
            :href="nav.href"
            :target="nav.target && nav.target"
            :rel="nav.rel && nav.rel"
            class="text-base ease duration-200 text-gray-300 hover:text-analogteal"
            >{{ nav.name }}</a
          >
        </div>
      </nav>
      <div class="mt-8 flex justify-center space-x-6">
        <a
          ...
          class="ease duration-200 text-gray-300 hover:text-analoggreen"
        >
          <span class="sr-only">{{ soc.name }}</span>
          <component :is="soc.icon" class="h-6 w-6" aria-hidden="true" />
        </a>
      </div>
      <p class="mt-8 text-center text-base text-gray-400">
        &copy; Copyright {{ new Date().getFullYear() }} Anya-Chan. All rights
        reserved.
      </p>
    </div>
  </footer>
...
const social = [
  {
    name: "Twitter",
    href: "https://twitter.com/AnyaChanBot",
